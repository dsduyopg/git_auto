...
# ---- 1. 停止并移除所有已登记的服务 ----
if [ -d "${CONF_DIR}/repos" ] && command -v systemctl >/dev/null 2>&1; then
    ...
    systemctl stop git-autosync-fetch.timer 2>/dev/null
    systemctl disable git-autosync-fetch.timer 2>/dev/null
fi
